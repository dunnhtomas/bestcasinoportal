# Professional Cloudflare Management and Cache Purging Solution
# CTO-Level Implementation for bestcasinoportal.com
...
ZONE_PURGE_CACHE_URL = f"{CLOUDFLARE_API_BASE}/zones/{ZONE_ID}/purge_cache"
ZONE_SETTINGS_URL = f"{CLOUDFLARE_API_BASE}/zones/{ZONE_ID}/settings"
ZONE_DNS_RECORDS_URL = f"{CLOUDFLARE_API_BASE}/zones/{ZONE_ID}/dns_records"
...
class CloudflareManager:
    ...
    def purge_everything(self) -> Dict[str, Any]:
    ...
    def purge_specific_urls(self, urls: List[str]) -> Dict[str, Any]:
        """Purge specific URLs from cache"""
        url = f"{self.base_url}/zones/{self.zone_id}/purge_cache"
        data = {"files": urls}
        
        print(f"🔄 Purging {len(urls)} specific URLs...")
        response = requests.post(url, headers=self.headers, json=data)
        
        if response.status_code == 200:
            result = response.json()
            print(f"✅ URL-specific cache purged successfully")
            return {"success": True, "result": result}
        else:
            print(f"❌ URL-specific cache purge failed: {response.status_code} - {response.text}")
            return {"success": False, "error": response.text}
    ...
    def update_cache_settings(self, cache_level: str = "aggressive") -> Dict[str, Any]:
        """Update cache settings"""
        url = f"{self.base_url}/zones/{self.zone_id}/settings/cache_level"
        data = {"value": cache_level}
        
        response = requests.patch(url, headers=self.headers, json=data)
        
        if response.status_code == 200:
            print(f"✅ Cache level updated to: {cache_level}")
            return {"success": True, "result": response.json()}
        else:
            print(f"❌ Cache level update failed: {response.status_code} - {response.text}")
            return {"success": False, "error": response.text}
    ...
    def verify_domain_status(self) -> Dict[str, Any]:
        ...
        for url in test_urls:
            try:
                ...
                print(f"✅ {url}: HTTP {response.status_code} ({len(response.content)} bytes)")
...
def main():
    ...
    print("\n1️⃣ Checking current zone settings...")
    ...
    if dns_records["success"]:
        ...
        for record in records:
            if record["type"] in ["A", "CNAME"]:
                print(f"✅ {record[
name]}: {record[type]} -> {record[content]} (Proxied: {record[proxied]})")
    
    # 3. Purge entire cache
    print("\n3️⃣ Purging entire cache...")
    purge_result = cf.purge_everything()
    
    # 4. Wait for cache purge to propagate
    print("\n4️⃣ Waiting for cache purge to propagate...")
    for i in range(5, 0, -1):
        print(f"⏳ Waiting {i} seconds...")
        time.sleep(1)
    
    # 5. Verify domain status
    print("\n5️⃣ Verifying domain accessibility...")
...
NO}")
    ...
    if cache_purged and successful_urls:
        ...
        print("\n⚠️  ISSUES DETECTED: Manual investigation required")
        return False
